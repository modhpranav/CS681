...
public class BostonHousingParallelStream {
...
//        Data Processing #2
        dataProcessing2(data.parallelStream().toList());

//        Data Processing #3
        dataProcessing3(data.parallelStream().toList());

//        Data Processing #4
        dataProcessing4(data.parallelStream().toList());

//        Data Processing #4
        dataProcessing5(data.parallelStream().toList());
    ...
    public static List<String[]> findRowsNearToChas(List<String[]> data) {
        List<String[]> housesNearChas = data.parallelStream()
    ...
    public static void dataProcessing2(List<String[]> data){
        ...
        int totalRows = (int) data.stream().parallel().count();

        int top10percent = (int) Math.ceil(totalRows * 0.1);

        List<String> areasLowCrimeRate = data.stream().parallel()
                .skip(1)
        ...
        List<String> areasLowPTRatio = data.stream().parallel()
                .skip(1)
        ...
        double[] maxPriceNoxRooms = new double[]{data.stream().parallel()
                .skip(1)
                .mapToDouble(house -> Double.parseDouble(house[3].replaceAll("\"", "")))
                .summaryStatistics()
                .getMax()};

        double[] minPriceNoxRooms = new double[]{data.stream().parallel()
                .skip(1)
                .mapToDouble(house -> Double.parseDouble(house[3].replaceAll("\"", "")))
                .summaryStatistics()
                .getMin()};

        double[] avgPriceNoxRooms = new double[]{data.stream().parallel()
                .skip(1)
                .mapToDouble(house -> Double.parseDouble(house[3].replaceAll("\"", "")))
                .summaryStatistics()
                .getAverage()};

        double[] maxNox = new double[]{data.stream().parallel()
                .skip(1)
                .mapToDouble(house -> Double.parseDouble(house[4]))
                .summaryStatistics()
                .getMax()};

        double[] minNox = new double[]{data.stream().parallel()
                .skip(1)
                .mapToDouble(house -> Double.parseDouble(house[4]))
                .summaryStatistics()
                .getMin()};

        double[] avgNox = new double[]{data.stream().parallel()
                .skip(1)
                .mapToDouble(house -> Double.parseDouble(house[4]))
                .summaryStatistics()
                .getAverage()};

        double[] maxRooms = new double[]{data.stream().parallel()
                .skip(1)
                .mapToDouble(house -> Double.parseDouble(house[5]))
                .summaryStatistics()
                .getMax()};

        double[] minRooms = new double[]{data.stream().parallel()
                .skip(1)
                .mapToDouble(house -> Double.parseDouble(house[5]))
                .summaryStatistics()
                .getMin()};

        double[] avgRooms = new double[]{data.stream().parallel()
                .skip(1)
                .mapToDouble(house -> Double.parseDouble(house[5]))
                .summaryStatistics()
                .getAverage()};
    ...
    public static void dataProcessing3(List<String[]> data){
        ...
        OptionalDouble maxMedianVal = data.stream().parallel()
                .skip(1)
                .mapToDouble(house -> Double.parseDouble(house[13]))
                .sorted()
                .skip(data.size() / 2)
                .findFirst();
        OptionalDouble minMedianVal = data.stream().parallel()
                .skip(1)
                .mapToDouble(house -> Double.parseDouble(house[13]))
                .sorted()
                .limit(data.size() / 2)
        ...
        double avgDistance = data.stream().parallel()
                .skip(1)
    ...
    public static void dataProcessing4(List<String[]> data) {
        ...
        double[] indus = data.stream().parallel()
                .skip(1)
        ...
        double indusSumOfSquares = Arrays.stream(indus).parallel()
                .map(d -> d - indusMean)
                .map(d -> d * d)
                .sum();
    ...
    public static void dataProcessing5(List<String[]> data){
        ...
        String[] townWithHighestBlacks = data.stream().parallel()
                .skip(1)
                .max(Comparator.comparingDouble(row -> Double.parseDouble(row[12])))
                .orElseThrow(() -> new RuntimeException("No town found"));

        double[] distances = Arrays.stream(townWithHighestBlacks).parallel()
                .skip(3)
                .limit(5)
                .skip(1)
        ...
        double weightSum = Arrays.stream(weights).parallel().sum();
